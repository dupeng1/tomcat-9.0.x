...
public abstract class WebappClassLoaderBase extends URLClassLoader
     ...
     *
     * ResourceEntry的缓存，用于加载的类和资源，按资源路径键，而不是二进制名称。
     * Path被用作键，因为资源可能由二进制名称(类)或路径(其他资源，如属性文件)请求
     * 并且从二进制名称到路径的映射是明确的，但反向映射是不明确的。
     */
     ...
     *
     * 如果我们想要破坏父委派模式自定义类加载器，则必须重写父类的loadClass方法
     * 如果我们不破坏父委派模式自定义加载器，就要实现findClass方法
     * @param name The binary name of the class to be loaded
    ...
    public Class<?> loadClass(String name, boolean resolve) throws ClassNotFoundException {
         ...
         *  基于以上有一个问题，对于一些非基础类库，为什么要先让webappClassLoader先去加载呢？
         *
         * 假设应用a依赖1.0版本的x.jar，而应用b依赖2.0版本的x.jar。为了保证两个应用的隔离性，
         * 首先要做的就是保证两个应用各自对应不同的webAppClassLoader实例。如果这两个
         * webAppClassLoader实例在加载x.jar的时候，直接向上委托，那么最终只会加载一个版本的x.jar。
         *
         *  --从上面，我们可以了解到：
         ...
         *
         *  发展会带来创新，创新就会带来变革，jdbc与tomcat打破了这个自古相传的机制。
        ...
        synchronized (JreCompat.isGraalAvailable() ? this : getClassLoadingLock(name)) {
            ...
            /**1、先从本地缓存中找当前完全限定名name对应的class对象有没有*/
            ...
            if (clazz != null) {
                ...
                if (resolve) {
                    /**
                     * resolveClass()用于对 Class 进行 链接，也就是把单一的 Class 加入到有继承关系的类树中。
                     * 如果你想在类被加载到 JVM 中时就被链接（Link），那么可以在调用 defineClass()
                     * 之后紧接着调用一个 resolveClass() 方法，当然你也可以选择让 JVM 来决定什么时候
                     * 才链接这个类（通常是真正被实实例化的时候）。
                     *
                     *  链接一个类分3步：
                     * 1、验证 ：被加载的类内部结构是否正确，比如，是否含有cafe babe
                     * 2、准备 ：为类变量分配内存并设置初始值
            ...
            /**
             * 路径转资源文件，
             * 如com.sun.org.apache.xerces.internal.jaxp.SAXParserFactoryImpl
             * 转com/sun/org/apache/xerces/internal/jaxp/SAXParserFactoryImpl.class
             * */
            String resourceName = binaryNameToPath(name, false);
            /**3、获取jdk内置类加载器，条件就是：向上委托，直到parent=null为主，其实就ExtClassLoader符合*/
            ...
            try {
                ...
                if (securityManager != null) {
                    ...
                    /**
                     * jar:file:/C:/Java/jdk1.8.0_211/jre/lib/rt.jar!/com/sun/org/apache/xerces/internal/jaxp/SAXParserFactoryImpl.class
                     * 如果resourceName在extClassLoader及其父类加载器加载的路径范畴内，url会返回一个具体路径
                     */
                    url = javaseLoader.getResource(resourceName);
                }
                /**如果url不为空的话，就尝试使用jse内置的类加载器去加载（javaseLoader为ExtClassLoader）*/
            ...
            /**
             * 只有满足 tryLoadingFromJavaseLoader 条件，才会委托父类加载器加载，否则都是自己先加载
             */
            if (tryLoadingFromJavaseLoader) {
                try {
                    /**
                     * WepAppClassLoader找不到就交给ExtClassLoader来加载（这里直接绕过了AppClassLoader），
                     * 目的是防止AppClassLoader加载应用1中的xxx时，又去加载应用2中的xxx时，
                     * 如果1中的xxx和2中的xxx名称一样但是版本不一样而导致的AppCLassLoader只会加载1个，
                     * 这就有问题了，两个应用的class本来应该是隔离的，但是却搞成一样了，就很离谱！
                     * 所以这里绕过AppClassLoader，第一次打破双亲委派机制！！！！！！！！！！！
                     * 其loadClass会调用ClassLoader#loadClass(String name, boolean resolve)方法
                     * 这个方法也会先从jvm里找，如果没有加载过，就判断loader的parent是否为null，
                     * 如果不为null，继续委托父加载器，反之，如果为null，就交给顶层的BootstrapClassLoader来加载，
                     * 目的是为了加载java.lang.Object类，防止webApp污染同限定名的Object类
                     */
            ...
            /**
             * 默认false，如果当前name（类的完全限定名）不再Ext类加载器加载的路径上，这里会判断是否开启了委托代理
             * 如果开启且当前url资源的路径不是tomcat所依赖的特有路径的话（filter方法做的事情），那就交给父类加载器去加载，
             * 这个顺序是依然遵循传统的父委派模式，先父后子
             *   AppClassLoader
             *   CommonClassLoader
             *   SharedClassLoader
             * 如果父类上还找不到，继续往下，交给当前的类加载器也就是ParallelWebappClassLoader去加载
             * */
            ...
            try {
                /**
                 * 如果没有委托父类的话，就使用{@link WebappClassLoaderBase#findClass(String)}去加载类
                 * findClass()会去当前应用的WEB-INF/classes和WEB-INF/lib 目录下查找类。
                 */
            ...
            if (!delegateLoad) {
                ...
                try {
                    /**
                     * 若是WebappClassLoaderBase在/WEB-INF/classes、/WEB-INF/lib下还是查找不到class，
    ...
    public void start() throws LifecycleException {
        System.out.println("======> WebappClassLoaderBase start.");
        state = LifecycleState.STARTING_PREP;
        /**加载classes下的class文件,如果是一个Servlet的话，肯定是有的*/
    ...
    protected Class<?> findClassInternal(String name) {
        ...
        if (entry == null) {
            ...
            synchronized (resourceEntries) {
                ...
                if (entry2 == null) {
                    resourceEntries.put(path, entry); // 缓存下
//                    System.out.println("======> classPath："+path+",修改时间："+new Date(entry.lastModified));
                } else {
                    entry = entry2;
                }
            }
