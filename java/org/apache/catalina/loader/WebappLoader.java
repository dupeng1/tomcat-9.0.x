...
public class WebappLoader extends LifecycleMBeanBase
     ...
     * loaderClass：ParallelWebappClassLoader
     */
     ...
     * 应用相关的class或者依赖的jar改变时触发热加载
     * 其实就是重新new一个类加载器，把改变的类再重新加载一遍
     */
    @Override
    public void backgroundProcess() {
        /**
         * 如果应用配置了热加载reloadable=true且应用下面的资源确实有改动
         * 那么就调用context的reload方法对当前应用重新加载（先停在启）
         */
    ...
    /**
     * Create associated classLoader . 创建WebappClassLoaderBase自定义类加载器实例
     * 最终类型是{@link ParallelWebappClassLoader} 并行类加载器
     */
    private WebappClassLoaderBase createClassLoader()
        ...
        if (parentClassLoader == null) {
            ...
            System.out.println("parentClassLoader : "+ parentClassLoader.hashCode()+","+parentClassLoader);
        } else {
            context.setParentClassLoader(parentClassLoader);
        }

        /**这玩意定死的，必然最后出去的都是ParallelWebappClassLoader类型的*/
