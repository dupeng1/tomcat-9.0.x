 ...
 * Context的启动事件监听器，用于配置该Context的属性和相关的已定义servlet。
...
public class ContextConfig implements LifecycleListener {
    ...
    /**
     * Cache of default web.xml fragments per Host 每个主机的默认web.xml片段缓存
     */
     ...
     * ServletContainerInitializer映射到它们表示感兴趣的类。
     */
    ...
    public void lifecycleEvent(LifecycleEvent event) {
        ...
        // Process the event that has occurred 如果StandardContext的state为CONFIGURE_START_EVENT，则触发
        if (event.getType().equals(Lifecycle.CONFIGURE_START_EVENT)) {
            /**启动配置（读取应用相关的部署描述符对象，然后最用到Context对象中）*/
            configureStart();
        } else if (event.getType().equals(Lifecycle.BEFORE_START_EVENT)) {
            /**修正context的docBase，如果file为war，就解压*/
            beforeStart();
    ...
    /**
     * Process the default configuration file, if it exists. 处理默认配置文件(如果存在)。
    ...
    /**
     * Adjust docBase. 修正应用的docBase（web应用的文件路径）
    ...
    protected void fixDocBase() throws IOException {
        ...
        /**
         * 判断下应用的docBase是否以当前localhost的appBase地址开头的
         * 如果不是的话，说明应用使用了绝对路径，那就不能用appBase去拼接地址了，只能用应用的绝对路径了
         * 如果不这样做的话，后面会找不到应用在哪
         */
        ...
        if (docBaseAbsolute.toLowerCase(Locale.ENGLISH).endsWith(".war") && !docBaseAbsoluteFile.isDirectory()) {
            ...
            /**如果应用没有指定.war,这里默认给拼接上，然后判断下有没有war，tomcat作者真细，这都考虑在内了*/
            ...
            /**
             * 如果正好歪打正着，就接着解压，一般我们单独配置context的时候，是不会用war的
             * 话说回来，一般我们部署应用就往webapps下面扔，也不会做过多配置的事情
             * 现在都是约定大于配置，程序员是不喜欢配置那一套的，因为太繁琐了，人都是懒惰的！
             */
    ...
    protected synchronized void beforeStart() {
        ...
        try {
            fixDocBase(); // 修整Web应用的docBase（文件路径，有相对也有绝对的）
        } catch (IOException e) {
            log.error(sm.getString(
    ...
    /**
     * Process a "contextConfig" event for this Context. 为该Context处理一个“contextConfig”事件。
     */
    protected synchronized void configureStart() {
        ...
        /**
         * 读取当前应用的web配置项（可能是web.xml，可能是servlet3.0注解，可能是走的SCI等）
         * 总之这里面会做一些context的配置工作，比如对每一个context都应用全局conf/web.xml中的配置，
         * 配置会给每一个context无差别的添加默认的XXXServlet和对应的mapping
         * 比如JspServlet等
         */
        webConfig();
        /**添加JSP解析初始化器*/
     ...
     * 对于全局web.xml文件，其中有重复的配置，最特定的级别优先。Ie应用程序的web.xml优先于主机级或全局web.xml文件。
     */
    protected void webConfig() {
         ...
         * 任何东西都可以覆盖全局和主机默认值。这是由两部分实现的，句柄作为一个web片段，
         * 它被添加到其他所有东西之后，因此其他所有东西都具有优先级
         * 标记servlet为可重写，因此SCI配置可以替换默认配置
         */
         ...
         * 注释扫描的规则并不像人们想象的那么明确。Tomcat实现了以下过程:-根据SRV.1.6.2,
         * Tomcat将扫描注释，而不管web.xml中声明的是哪个Servlet规范版本。专家组已经证实，
         * 这是预期的行为。 参照http://java.net/jira/browse/SERVLET_SPEC-36，
         * 如果主web.xml被标记为元数据完整，则SCIs的jar仍然会被处理。
         * 如果metadata-complete=true并且指定了绝对排序，则在排序中被排除的jar也将被排除在SCI处理之外。
         * 如果一个SCI有一个@HandlesType注释，那么所有的类(除了那些在jar中被排除在绝对排序之外的类)
         * 都需要被扫描以检查它们是否匹配。
         * 什么是SCI：全称 ServletContainerInitializer，是一个用于接收Web应用在启动阶段通知的接口，
         * 再根据通知进行一些编程式的处理，比如动态注册Servlet、Filter，添加Listener等
         */
        /**创建webXml解析器,分别实例化webDigester和webFragmentDigester*/
        WebXmlParser webXmlParser = new WebXmlParser(context.getXmlNamespaceAware(),
                context.getXmlValidation(), context.getXmlBlockExternal());

        /**WebXml: 为web.xml 和 web-fragment.xml 共用的业务模型*/
        Set<WebXml> defaults = new HashSet<>();
        /**
         *
         * /conf/web.xml,tomcat默认的web.xml,
         * 提供了两个servlet：
         * ① DefaultServlet
         * ② JspServlet
         * */
        ...
        /**找到tomcat默认的web.xml,将其属性作用到每个context上*/
        Set<WebXml> tomcatWebXml = new HashSet<>();
        /**在当前应用文件夹下找：/WEB-INF/tomcat-web.xml,如果有解析*/
        tomcatWebXml.add(getTomcatWebXmlFragment(webXmlParser));

        /**创建应用对象context的webXml*/
        WebXml webXml = createWebXml();

        /**
         *  Parse context level web.xml 解析上下文级别的web.xml
         * 获取web应用（文件夹下）的/WEB-INF/web.xml文件流
         */
        InputSource contextWebXml = getContextWebXmlSource();
        /**单独解析，不带fragment（片段的）的*/
        ...
        /**
         * 获取当前web应用（StandardContext）的servlet上下文
         * 这里就不得不说ServletContext这个类了，地位很为重要，
         * 首先，在应用中，多个Servlet之间共享的数据都可以存放在上下文中
         * 另外，上下文还可以添加监听器、过滤器和servlet，也就是相当于
         * web.xml的功能，换句话说，有了ServletContext（配合SCI使用），就不用再搞web.xml那一套了
         * 所以现在SpringBoot框架打包后的war中，WEB-INF/下是看不到web.xml的
         */
        ServletContext sContext = context.getServletContext();

        /**
         * Ordering is important here （这里的排序是很关键很重要的）
         * Step 1. Identify all the JARs packaged with the application and those
         * provided by the container. If any of the application JARs have a
         * web-fragment.xml it will be parsed at this point. web-fragment.xml
         * files are ignored for container provided JARs.
         *
         * 步骤1。标识与应用程序打包的所有jar和容器提供的jar。
         * 如果任何应用程序jar有一个web-fragment.xml，它将会被digest库解析。
         * web-fragment.xml ： web应用xml部署描述符片段文件
         */
         ...
         * 步骤2： 对fragments进行排序
         * 排序分两部分：
         * 1. 主应用web.xml（webXml）中配置了fragment的绝对排序，那fragment就按这个走
         * 2. 主应用web.xml（webXml）中没有配置绝对排序，那就按web-fragment.xml的相对排序来
         * 3. 如果都没有设置顺序，fragment顺序按正常来
         */
         ...
         * 步骤3：寻找（收集）所有实现了SCI接口的实例
         * 执行条件：如果web.xml 和 web-fragment.xml解析成功，才会执行步骤3
         */
        if (ok) {
        ...
        /**
         * metadata-complete属性表示部署时当前的配置是否完全，值为true，
         * 表示完全，只会应用web.xml的配置，而不会去扫描类似@WebServlet,
         * @WebFilter,@WebListener等注解和web-frame.xml配置。
         * 默认值是metadata-complete=false,即不完全，会对项目中类进行扫描，
         * 是否有相关的注解配置，同时也会加载web-frame.xml等插件配置。
         */
        if  (!webXml.isMetadataComplete() || typeInitializerMap.size() > 0) {
            /**
             * Steps 4 & 5.
             * 步骤4 ： 扫描 /WEB-INF/classes下面的所有class，处理类上符合servlet3.0规范的注解及@HandlesTypes注解
             * 步骤5 ： 遍历orderedFragments里的所有jar，处理@HandlesTypes注解
             * （主要功能就是servlet3.0特性，注解替代web.xml，典型的就是spring-webmvc模块）
             * webXml ：主应用的xml
             * orderedFragments : 主应用依赖的jars包中的web-fragment.xml解析且排过序的webXml对象集
             */
            processClasses(webXml, orderedFragments);
        }

        /**只有当应用的web.xml中的<web-app metadata-complete="false" /> 时才会触发该条件*/
        if (!webXml.isMetadataComplete()) {
            /***
             * Step 6. Merge web-fragment.xml files into the main web.xml file
             * 步骤6： 将当前应用下的所有的xml部署描述符片段对象的属性合并到应用的webXml对象属性中
             * 执行条件： 应用的web.xml 和 应用中jars的web-fragment.xml 及 sci处理都成功才会执行该步骤
             */
            if (ok) {
            ...
            /**
             * Step 7a merge tomcat-web.xml
             * 合并应用下的/WEB-INF/tomcat-web.xml
             */
             ...
             * 合并tomcat中所有应用共享的的/conf/web.xml
             */
            webXml.merge(defaults);

            /**
             * 步骤8 Convert explicitly mentioned jsps to servlets
             * 将webXml中涉及到的jsp转换为相应的servlet
             */
            if (ok) {
                convertJsps(webXml);
            }
             ...
             * 步骤9：将合并的 web.xml 对象属性配置到StandardContext对象中
             */
            if (ok) {
        ...
        // context 将ServletContainerInitializer配置应用到上下文
        if (ok) {
    ...
    protected void processClasses(WebXml webXml, Set<WebXml> orderedFragments) {
        ...
        if (ok) {
            /**获取java web开发项目中/WEB-INF/classes下面的资源数组*/
            WebResource[] webResources = context.getResources().listResources("/WEB-INF/classes");
            for (WebResource webResource : webResources) {
                /**
                 * Skip the META-INF directory from any JARs that have been
                 * expanded in to WEB-INF/classes (sometimes IDEs do this).
                 * META-INF是jar包的描述信息所在的目录，它下面是不会存放class的，所以扫描它没意义
                 */
                if ("META-INF".equals(webResource.getName())) {
                    continue;
                }
                /**处理当前资源路径中类的注解（这个方法是一个递归，直到WebResource对应的是文件）*/
    ...
    private void configureContext(WebXml webxml) {
        ...
        for (ServletDef servlet : webxml.getServlets().values()) {
            /**构造一个StandardWrapper，同时添加StandardWrapperValve到tomcat容器管道中*/
            ...
            /**StandardContext添加StandardWrapper*/
    ...
    private WebXml getTomcatWebXmlFragment(WebXmlParser webXmlParser) {
        ...
        /**Constants.TomcatWebXml:/WEB-INF/tomcat-web.xml*/
    ...
    private WebXml getDefaultWebXmlFragment(WebXmlParser webXmlParser) {
        ...
        synchronized (host.getPipeline()) {
            ...
            if (globalWebXml == null) {
                ...
                /**如果globalWebXml不等于空（tomcat home/conf/ 目录下的web.xml）*/
     ...
     * 扫描jar以获得ServletContainerInitializer实现。
     */
    protected void processServletContainerInitializers() {
        /**检测到的SCI Class实例列表*/
        List<ServletContainerInitializer> detectedScis;
        try {
            /**java web项目SPI加载器，主要就是从META-INF/services/下找所有实现了ServletContainerInitializer的类*/
        ...
        for (ServletContainerInitializer sci : detectedScis) {
            ...
            for (Class<?> type : types) {
                ...
                /**拿到这个类对应的sci初始化器，一开始typeInitializerMap没东西*/
                ...
                /**将当前类对应的sci添加进去，也就是当前方法只是找到sci映射，并没有去执行sci逻辑*/
                scis.add(sci);
            }
        }
    }
    ...
    /**
     * Scan JARs that contain web-fragment-1.xml files that will be used to
     * configure this application to see if they also contain static resources.
     * If static resources are found, add them to the context. Resources are
     * added in web-fragment-1.xml priority order.
     *
     * 扫描包含用于配置此应用程序的web-fragment-1.xml文件的jar，看看它们是否也包含静态资源。
     * 如果找到静态资源， 将它们添加到上下文中。资源按web-fragment-1.xml优先级顺序添加。
     * 将扫描静态资源的片段集
     ...
     * /META-INF/web-fragment-1.xml to the resulting Map. web-fragment-1.xml files
     * will be parsed before being added to the map. Every JAR will be added and
     * <code>null</code> will be used if no web-fragment-1.xml was found. Any JARs
    ...
    protected Map<String,WebXml> processJarsForWebFragments(WebXml application,
        ...
        JarScanner jarScanner = context.getJarScanner(); // StandardJarScanner
        ...
        if (absoluteOrder != null && absoluteOrder.isEmpty() && !context.getXmlValidation()) {
             ...
             * 当绝对排序有标签但是内容为空且未启用验证时，jar包中的web-fragment.xml跳过解析
             * 一般在主应用web.xml中有标签的，其内容肯定不为空的，所以，parseRequired可以认定基本上就是true，
             */
            parseRequired = false;
        }

        /**扫描jar时，遇到其中有web-fragment.xml描述符的就解析。*/
        FragmentJarScannerCallback callback = new FragmentJarScannerCallback(webXmlParser, delegate, parseRequired);
        /**
         * {@link org.apache.tomcat.util.scan.StandardJarScanner} 扫描应用WEB-INF/lib目下的jar，找到jar文件扔给callBack进行回调处理
         * 扫描jar的前提是，在tomcat工作目录下的conf/catalina.properties中的配置
         * tomcat.util.scan.StandardJarScanFilter.jarsToSkip 配置有值，且要扫描的jar不再这个jarsToSkip 集合里
         * 这里隐藏tomcat一个提高启动速度的优化点
         */
        ...
        /**所有应用相关的jar都会存到一个map中，最终把这个map也就是扫描jar的处理结果返回出去*/
    ...
    protected void processAnnotationsWebResource(WebResource webResource,
        ...
        if (webResource.isDirectory()) {
            ...
            /**如果是文件且以.class结尾的，就处理java class*/
    ...
    /**
     * 处理class上的注解（主要就是servlet3.0新出的几个注解，用来替代web.xml的）
     */
    ...
    /**
     * 这个就要分情况了，取出类上的注解，判断是不是@WebServlet @WebFilter @WebListener
     * 对号入座解析即可，其实就是通过注解的方式构建webXML
     * 以前需要繁琐的在web.xml里配制各种的servlet，servlet-mapping、filter、listener等
     * 现在不需要了，自从servlet3.0支持注解后，程序员开发web程序就更加容易了。
     * @param fragment
     * @param clazz WEB-INF/classes下面的
     */
    protected void processClass(WebXml fragment, JavaClass clazz) {
        AnnotationEntry[] annotationsEntries = clazz.getAnnotationEntries();
        if (annotationsEntries != null) {
            String className = clazz.getClassName();
            /**反射拿注解对象，遍历并只处理特定的几个@WebXXX注解*/
            for (AnnotationEntry ae : annotationsEntries) {
                String type = ae.getAnnotationType();
                if ("Ljavax/servlet/annotation/WebServlet;".equals(type)) {
                    /**处理servlet注解，还涉及到mapping地址的添加*/
                    processAnnotationWebServlet(className, ae, fragment);
                }else if ("Ljavax/servlet/annotation/WebFilter;".equals(type)) {
                    /**处理filter注解，还涉及到添加URLPattern*/
    ...
    protected void checkHandlesTypes(JavaClass javaClass,
        ...
        // Skip this if we can 如果类没有作为sci的入参的话，直接返回，其实就是检查类是否匹配@HandlesTypes
     ...
     * 处理类上的WebFilter注解，有的话添加到WebXml中
