...
final class StandardHostValve extends ValveBase {
    ...
    public final void invoke(Request request, Response response)
        ...
        System.out.println("======》StandardHostValve#invoke方法被调用");
        // Select the Context to be used for this Request
        /**拿到当前请求关联的context容器对象*/
        Context context = request.getContext();
        System.out.println("------》基于请求获取指定的应用(webapps下面的目录名)："+((StandardContext) context).getBaseName());
        if (context == null) {
        ...
        try {
            ...
            try {
                if (!response.isErrorReportRequired()) {
                    /**继续找管道中的第一个Value对象进行invoke调用（注：host下一个是context，context下一个是wrapper）*/
                    System.out.println("-------》"+context.getPipeline().getFirst());
