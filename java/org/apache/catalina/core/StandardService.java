...
public class StandardService extends LifecycleMBeanBase implements Service {
     ...
     * 此服务拥有的服务节点，及反向关联，一个server包含多个services，反过来，一个services对应一个server
     */
    ...
    /**
     * 包含 Host和 Context，接到请求后扔给相应的Host在相应的Context里处理。
     * Service默认的Container对象
     */
    ...
    /**
     * Mapper. 映射器，就url和主机之间的映射关系，即基于请求中的url，tomcat是如何找到对应的host下面的具体的应用中的servlet的
     */
    ...
    /**
     * Mapper listener.
     * 组合，强关联，MapperListener内部维护了一个service实例，最终MapperListener工作的结果会保存到mapper对象中
     */
    ...
    protected void startInternal() throws LifecycleException {
        ...
        /**跟StandardServer组件一样，Service组件启动的第一件事情就是设置生命周期状态为：STARTING*/
        ...
        if (engine != null) {
            synchronized (engine) {
                /**
                 * Engine启动还是很关键的，其涉及到了启动Host和Context
                 * 其中Host启动时会触发{@link org.apache.catalina.startup.HostConfig#lifecycleEvent(LifecycleEvent)}并行部署应用（xml描述符、war包及目录）
                 * 其中Context启动时会触发{@link org.apache.catalina.startup.ContextConfig#lifecycleEvent(LifecycleEvent)}加载servlet
                 * Servlet容器引擎启动后，整个Catalina容器应用的部署和应用class类加载就全部完成了。
                 * 剩下的就是看Connector和CoyoteAdapter发挥作用了！
                 */
                engine.start();
            }
        }
        ...
        synchronized (connectorsLock) {
            for (Connector connector: connectors) {
                ...
                if (connector.getState() != LifecycleState.FAILED) {
                    /**启动连接器（endpoint、Acceptor、Poller等相继启动）*/
                    connector.start();
                }
            }
        }
    ...
    protected void initInternal() throws LifecycleException {
        ...
        /**init主要做了一件事情，就是将mapperListener封装成bean注册到MBServer容器中*/
        mapperListener.init();
