...
final class StandardWrapperValve extends ValveBase {
    ...
    public final void invoke(Request request, Response response)
        ...
        System.out.println("======》StandardWrapperValve#invoke方法被调用");
        ...
        /** 将请求、wrapper（tomcat最小的容器，对应一个servlet）、servlet（就是context中的我们部署的应用）封装成请求链*/
            ...
            if ((servlet != null) && (filterChain != null)) {
                ...
                if (context.getSwallowOutput()) {
                    ...
                    if (request.isAsyncDispatching()) {
                        ...
                        /**最终servlet的service方法在这个过滤链中被调用*/
                        HttpServletRequest requestFacade = request.getRequest();
                        HttpServletResponse responseFacade = response.getResponse();
                        /**
                         * 调用过滤器其链{@link ApplicationFilterChain#doFilter(ServletRequest, ServletResponse)}
                         * 如果有filter，则递归调用doFilter，如果filter的数量等于0
                         * 则直接调用servlet的service方法
                         * 注意，这时候执行全就交给应用Servlet了，由于tomcat自身的request属性太多，大多数
                         * 都不是开发人员需要知道的，所以，在将request和response传给servlet之前，需要对二者进行包装
                         * 设计模式：门面模式。
                         */
