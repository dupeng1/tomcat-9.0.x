 ...
 * 实现容器的组件有
 * Engine：
 *   List<Host> hosts ; // 虚拟主机对象，默认是localhost
 * Host:
 *   List<Context> contexts; // 应用上下文对象，一般来说，一个host下我们会部署多个应用（war）
 * Context:
 *   List<Wrapper> wrappers  // 同一类型的servlet放在一起构成一个wrapper对象
 * Wrapper:
 *  单个Servlet定义;
 * Servlet:
 *   具体的web应用
...
public class StandardContext extends ContainerBase
    ...
    public StandardContext() {
        super();
        /**设置当前context容器的基础阀门（其实就是管道中的最后一个阀门，每一个容器都有一个默认的）*/
        pipeline.setBasic(new StandardContextValve());
        System.out.println("--------》StandardContext构造器，往pipeline中添加StandardContextValve实例");
     ...
     *
     * tomcat提供了delegate属性用于控制是否启用java委派模式，
     * 默认false（不启用），当设置为true时，tomcat将使用java的默认委派模式，这时加载顺序如下：
     *
     * (1).先从缓存中加载；
     ...
     * 与此容器相关联的Loader实现。Loader用来加载servlet class
     */
    private Loader loader = null;
    /**Tomcat定义的Loader实例的读写锁*/
     ...
     * 热加载 默认 false，如果xml里配置true的话，一旦检测class改变会卸载WebAppClassLoader然后重新new
     */
    private boolean reloadable = false;


    /**
     * Unpack WAR property. 默认解压war
     */
     ...
     * configuration information such as TLDs or web-fragment-1.xml files.
     */
    ...
    public ClassLoader getParentClassLoader() {
        ...
        if (getPrivileged()) { // 默认false
            return this.getClass().getClassLoader();
        } else if (parent != null) {
            /**从当前context的父容器host对象中拿父类加载，这个不是别就是sharedClassLoader*/
    ...
    /**
     * @return the privileged flag for this web application.此web应用程序的特权标志
     */
    @Override
    ...
    public boolean loadOnStartup(Container children[]) {
        ...
        for (ArrayList<Wrapper> list : map.values()) {
            for (Wrapper wrapper : list) {
                try {
                    wrapper.load(); // 启动tomcat时就加载相应的servletWrapper
     ...
     * 启动context（这个里面有个方法很关键，就是读取web.xml文件中的配置，
     * 然后把所有的servlet都收集起来，后续基于servletClass进行反射实例化对象）
     */
    @Override
    protected synchronized void startInternal() throws LifecycleException {
        ...
        if (ok) {
            /**启动资源组件{@link StandardRoot#start()},这一步也很重要，因为应用类加载器要用到资源·*/
            resourcesStart();
        }

        if (getLoader() == null) {
            /**构建一个loader，每一个loader都会为当前应用创建一个类加载器，注意这个是web应用的loader不是classLoader*/
            WebappLoader webappLoader = new WebappLoader();
            /**配置代理模式，默认false，如果为true，classloading首先委托给父类加载器*/
            webappLoader.setDelegate(getDelegate());
            /**设置完后，后面会调用loader的start方法，因为Loader也实现了Lifecycle接口*/
        ...
        /**
         * Binding thread 将当前应用的类加载器绑定到当前线程上
         * 在loader还没有start之前，当前应用的类加载器等于null
         */
        ...
        try {
            if (ok) {
                ...
                /**
                 * 启动我们的loader组件，如果有的话,这里必须有，因为上面已经设置过了，loader是{@link WebappLoader}
                 * 而且{@link WebappLoader}还实现了{@link Lifecycle}接口
                 * 所以下面的判断必然通过，通过后就是启动loader了
                 * 实际会调用{@link WebappLoader#startInternal()}
                 */
                Loader loader = getLoader();
                if (loader instanceof Lifecycle) {
                    ((Lifecycle) loader).start(); /**{@link WebappClassLoaderBase#start()}*/
                }
                 ...
                 * 通过连续调用unbindThread和bindThread，我们将当前线程CCL设置为当前应用程序的类加载器
                 * tomcat7是WebappClassLoader，8之后就是ParallelWebappClassLoader了
                 */
                unbindThread(oldCCL);
                oldCCL = bindThread();
                ...
                // Notify our interested LifecycleListeners 再次触发ContextConfig的回调
            ...
            // Load and initialize all "load on startup" servlets 加载并初始化所有“启动时加载”servlet
            if (ok) {
            ...
            /***
             * Unbinding thread
             * 待一个应用所有的资源加载完之后，卸载当前线程上下文绑定的ParallelWebappClassLoader
             * 将其在换回CatalinaClassLoader
             */
            unbindThread(oldCCL);
        }
    ...
    public void backgroundProcess() {
        ...
        if (loader != null) {
            try {
                /**
                 * 应用加载器后台执行周期性任务，检查class或jar是否有变动
                 * 前提是server.xml配置文件中，context的reloadable配置为true
                 * 因为其默认是false
                 * 这里要区别autoDeploy和reloadable
                 * autoDeploy是热部署，执行容器是Host
                 * reloadable是热加载，执行容器是Context
                 */
    ...
    public ClassLoader bind(boolean usePrivilegedAction, ClassLoader originalClassLoader) {
        ...
        if (originalClassLoader == null) {
            if (usePrivilegedAction) {
                ...
                /**拿到线程上下文类加载，必然是catalinaClassLoader*/
                originalClassLoader = Thread.currentThread().getContextClassLoader();
            }
        }

        /**如果bind是在loader.start()之前，则webApplicationClassLoader是等于null的*/
        if (webApplicationClassLoader == null ||
             ...
             * 这个时候再切换线程上下文关联的类加载就没有意义了，所以直接返回null
             */
            return null;
        }
        ...
        if (usePrivilegedAction) {
            ...
            /**
             * 如果loader启动之后，应用程序也有了类加载器，那就将其设置到线程上下文中
             * 好处就是： 诸如spring框架在getBean时，会通过当前线程上下文类加载器去管理bean
             * 如果我们把Spring的jar扔到CommonLoader或者SharedLoader目录下，
             * 这就相当于打破了父委派模式，使得父类加载可以加载子类加载作用域下面的资源
             */
