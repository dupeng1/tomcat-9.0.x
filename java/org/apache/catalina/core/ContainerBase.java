 ...
 *
 * Container接口的抽象实现（设计模式：模板方法），提供几乎所有实现所需的通用功能。 扩展此基类的类必须实现对invoke()的替代。
 * 此抽象基类的所有子类都将支持 Pipeline 对象，该对象定义了对此类的invoke()方法接收到的每个请求执行的处理，
 * 利用“责任链”设计模式。 子类应该将自己的处理功能封装为Valve ，并通过调用setBasic()将此 Valve 配置到管道中。
 * 根据 JavaBeans 设计模式，此实现会针对单例属性的更改触发属性更改事件。
 * 此外，它会向使用addContainerListener()注册自己的侦听器触发以下ContainerEvent事件：
 *
...
public abstract class ContainerBase extends LifecycleMBeanBase implements Container {
     ...
     * Tomcat.
     * 使用该类的权限执行addChild。addChild可以用堆栈上的XML解析器调用，
     ...
     * 与此 Container 相关联的 Pipeline 对象。
     * 这个玩意有点类似于netty的channel的pipeline
     * 1、netty的管道中放的是一个handler的链条
     * 2、tomcat的容器管道中放的是各个实现子类（engine、host、context，wrapper）处理功能封装成的XXXValve实例
     * 3、Valve是一个接口，其有方法setNext(Valve valve)、getNext()等
     */
     ...
     * java jdk自带的一个类，用于监听某个对象属性的改变来触发相应信息
     */
     ...
     * 如果应用多的话，可以将默认1改为合适的值来提高启动子容器的速度
     */
     ...
     * 启动容器（属于容器的组件有：Engine、Host、Context、Wrapper）
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // Start our child containers, if any 启动我们的子容器，如果有的话
        Container children[] = findChildren();
        List<Future<Void>> results = new ArrayList<>();
        for (Container child : children) {
            /**
             * 如果容器有子容器的话，就起一个线程单独去启动子容器
             * 并行启动子容器
             * 比如Catalina在load时会解析conf/server.xml,其中会处理Engine节点，其下面有一个默认的Host子容器
             * 所以，在Catalina启动start方法时会逐一启动各个组件，启动Engine的时候会相应的启动其子容器Host
             */
        ...
        /**启动管道中的各个valve（阀门），比如Host容器下面就有N个valve*/
        if (pipeline instanceof Lifecycle) {
            ((Lifecycle) pipeline).start();
        }

        /**设置状态为：Lifecycle.START_EVENT，如果当前容器为StandardHost，则会触发其监听器{@link HostConfig#start()}方法*/
        setState(LifecycleState.STARTING);

        // Start our thread 启动后台任务，执行Engine容器的后台周期性任务
     ...
     * 每个容器都会执行自己的周期性任务，如应用热加载等。
     * 此方法将在此容器的类加载上下文中调用。意外的抛出将被捕获并记录。
     */
    @Override
    public void backgroundProcess() {
        ...
        while (current != null) {
            try {
                /**
                 * 执行每个容器中pipeline中的每个valve的后台任务和
                 */
        ...
        /**发布PERIODIC_EVENT事件 其中热部署就是在发布事件中*/
    ...
    /**
     * 容器对象toString方法，比如当前容器对象是StandardContext类型的，则打印该对象就是下面的字符串形式
     ...
     * 用于在固定延迟后调用此容器及其子容器的backgroundProcess方法。
     */
    ...
    private static class StartChild implements Callable<Void> {
        ...
        public Void call() throws LifecycleException {
            child.start(); /** 这个地方最终会调用{@link StandardHost#startInternal()}*/
            return null;
        }
    }
