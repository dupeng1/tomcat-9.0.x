 ...
 * 表示单个 servlet 定义的Wrapper接口的标准实现。 不允许有子 Container，父 Container 必须是 Context
...
public class StandardWrapper extends ContainerBase implements ServletConfig, Wrapper, NotificationEmitter {
    ...
    public StandardWrapper() {
        ...
        /**设置基础（链条里最后一个）阀门，阀门有一个invoke方法*/
        pipeline.setBasic(swValve);
        System.out.println("--------》StandardWrapper构造器，往pipeline中添加StandardWrapperValve实例");
     ...
     * 一个wrapper关联一个StandardWrapperFacade
     */
     ...
     * 这个servlet的启动时加载顺序值(负值表示第一次调用时加载)。
     */
    ...
    /**
     * Does this servlet implement the SingleThreadModel interface? 这个servlet实现了SingleThreadModel接口吗?
     */
     ...
     * 单个Servlet实例，如果程序员编写的Servlet没有实现{@link SingleThreadModel}接口，那它在Tomcat servlet容器中就是
     * 单例的，这种servlet如果内部有共享变量的话，会存在线程安全问题！
     * 反之，如果实现了{@link SingleThreadModel}接口，则包装器在分配Servlet实例的时候，会考虑使用多例，
     * 即同一时间多个线程访问了servlet（url），可能会发生一种类型的servlet在jvm中对应不同的实例，这些实例
     * 最终会保存在Stack栈数据结构中。
     */
     ...
     *
     * 分配此 Servlet 的一个已初始化实例，该实例已准备好调用其service()方法。 如果 servlet 类没有实现SingleThreadModel ，
     * （唯一的）初始化实例可能会立即返回。 如果 servlet 类实现SingleThreadModel ，
     * 则 Wrapper 实现必须确保在通过调用deallocate()解除分配之前不会再次分配此实例。
     *
    ...
    public Servlet allocate() throws ServletException {
        ...
        if (!singleThreadModel) {
            // Load and initialize our instance if necessary DCL！！！====> 双重检查锁，配合volatile
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        System.out.println("======> Wrapper start. servletClass ："+servletClass);
