...
public class CoyoteAdapter implements Adapter {
    ...
    public void service(org.apache.coyote.Request req, org.apache.coyote.Response res)
        ...
        try {
             ...
             *  request parameters 解析并设置 Catalina 和配置特定的请求参数
             * 非常关键的处理，有一个是填充request对象中的mappingData信息
             * 如果uri访问的是根路径，则会重定向，也就是CoyoteAdapter的service方法会调用两次
             */
            postParseSuccess = postParseRequest(req, request, res, response);
            /**
             * postParseSuccess等于false，说明发生重定向，重定向不会向下继续处理request，而是客户端再次发起一次请求
             * 只有postParseSuccess等于true时 （等于true才会真正意义上匹配到了wrapper），才会交给剩下的阀门继续处理
             */
            if (postParseSuccess) {
                ...
                /**
                 * 调用第一个容器组件（StandEngine）管道中的第一个阀门对象（StandardEngineValve的invoke方法
                 * {@link org.apache.catalina.core.StandardEngineValve#invoke(Request, Response)}
                 *
                 * 注意：
                 * 此时coyoteAdapter已经将coyote的request包装成catalina 的 request传递给容器了
                 * 此时coyoteAdapter已经将coyote的response包装成catalina 的 response传递给容器了
                 * 即做了一个接口参数适配
                 * 此时request中的mappingData就有数据了，后续的阀门工作起来就得心应手了
                 */
                connector.getService().getContainer().getPipeline().getFirst().invoke(
                        request, response);
            }
     ...
     * 在解析 HTTP 标头后执行必要的处理，以使请求响应对象能够传递到容器管道的第一个阀门进行处理。
    ...
    protected boolean postParseRequest(org.apache.coyote.Request req, Request request,
        ...
        /**
         * 从原生（coyote）请求对象中拿到请求的uri（统一资源标识符），uri是资源唯一的标识符，
         * 拿tomcat来说，一个uri只能对应一个context。
         * url是统一资源定位符，包含找到资源的所要用到的协议、主机和端口等。
         * url是uri的子集
         * */
        ...
        while (mapRequired) {
            // This will map the the latest version by default，默认情况下，这将映射最新版本
            /**
             * 这个地方很关键，也就是coyote适配器要基于请求的url去mapper中找到对应的context了，
       ...
       /**Possible redirect,这里可能发生重定向*/
       MessageBytes redirectPathMB = request.getMappingData().redirectPath;
       /**如果发生了重定向（只有访问context的根path时才会发生）*/
       if (!redirectPathMB.isNull()) {
            /**获取重定向的path，比如 uri是： /xd  则重定向的地址是: /xd/ */
     ...
     * 从请求中提取路径参数。这里假设参数的形式是path;name=value;name2=value2等。目前只对这个表单中的会话ID感兴趣。其他参数可以安全地忽略。
     *
     * @param req The Coyote request object
    ...
    protected void parsePathParameters(org.apache.coyote.Request req,
        ...
        /**
         * Performance optimisation. Return as soon as it is known there are no path parameters;
         * 如果没有分号就不提取参数了直接返回
         */
        if (semicolon == -1) {
            return;
