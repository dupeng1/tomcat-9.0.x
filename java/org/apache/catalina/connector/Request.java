 ...
 * catalina的请求对象，对coyote请求对象进行了一次包装
...
public class Request implements HttpServletRequest {
    ...
    /**
     * Mapping data.映射数据，这个很关键，即一次socket请求，会构造一个mappingData对象，
     * 通过mappingData对象可以找到请求对应的Host、context和 wrapper，找到wrapper就找到了servlet
     * 剩下的就是执行业务代码响应数据
     */
     ...
     * 创建一个与给定连接器关联的新Request对象。
     * @param connector 这个请求对象将始终与之关联的连接器。
     * 在正常使用中，该参数必须是非空的。在某些测试场景中，可能使用空连接器而不触发NPE。
     */
     ...
     * 返回一个ServletRequest实例，RequestFacade其实就是对catalina Request的包装
     * 而catalina Request 又是对coyote Request的包装
     * 其也实现了HttpServletRequest接口
     */
