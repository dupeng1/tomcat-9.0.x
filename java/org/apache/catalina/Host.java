 ...
 *
 * Host是一个容器，代表 Catalina servlet 引擎中的虚拟主机。 它在以下类型的场景中很有用：
 * 您希望使用拦截器来查看此特定虚拟主机处理的每个请求。
 * 您希望使用独立的 HTTP 连接器运行 Catalina，但仍希望支持多个虚拟主机。
 * 通常，在部署连接到 Web 服务器（例如 Apache）的 Catalina 时，您不会使用 Host，
 * 因为连接器将利用 Web 服务器的设施来确定应使用哪个上下文（甚至可能是哪个 Wrapper）来处理此问题要求。
 * 附加到主机的父容器通常是一个引擎，但也可能是其他一些实现，或者如果没有必要可以省略。
 * 附加到 Host 的子容器通常是 Context（代表一个单独的 servlet 上下文）的实现。就是我们所理解的虚拟主机
 *
...
public interface Host extends Container {
     ...
     * 返回该主机的应用程序根目录。这可以是一个绝对路径名、一个相对路径名或一个URL
     * 默认是：${catalina.base}/webapps
     */
     ...
     * deployment process.
     * 返回编译后的正则表达式，该正则表达式定义了自动部署过程中将忽略的主机appBase中的文件和目录。
     */
     ...
     * 用于启动和停止上下文(应用)的执行器。通过线程池来启动和停止。
     */
