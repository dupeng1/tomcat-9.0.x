 ...
 * limited.
 *
 * LockOutRealm 是一个Tomcat的Realm实现，它扩展了CombinedRealm，
 * 假如在某一段时间内出现很多验证失败，则它能够提供锁定用户的功能，已避免攻击者进行密码暴力破解
 *
 */
public class LockOutRealm extends CombinedRealm {
     ...
     * 连续用户认证失败后被锁定的次数。默认值为5。即用户5次不通过将会被锁定，锁定时间5分钟
     */
     ...
     * 认证失败次数过多后，用户锁定的时间(以秒为单位)。默认为300(5分钟)。
     */
     ...
     * 1000.
     * 未通过身份验证而保留在缓存中的用户数。随着时间的推移，缓存将增长到这个大小，可能不会缩小。默认值为1000。
     * 为了防止通过故意向无效用户发出请求(从而导致缓存增长)而发生DOS，限制了身份验证失败的用户列表的大小。
     */
     ...
     * 如果一个失败的用户因为缓存太大而被从缓存中删除，那么在缓存中缓存的时间(以秒为单位)至少会记录一条警告消息。默认为3600(1小时)。
     */
     ...
     * 上次认证失败的用户。条目将按照从最近到最近的访问顺序进行排序。
     */
    ...
    protected synchronized void startInternal() throws LifecycleException {
        ...
        // exceeds the specified size LRU
