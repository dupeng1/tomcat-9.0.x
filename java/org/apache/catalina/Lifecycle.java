 ...
 *
 * 组件生命周期方法的通用接口。 Catalina 组件可以实现此接口（以及它们支持的功能的适当接口），以便提供一致的机制来启动和停止组件
 * 这个接口设计的非常巧妙，不管你是server、engine、host等，对于tomcat来说都是组件，组件就有生命周期，比如init、start、stop等功能
 *
 * <br>
...
public interface Lifecycle {
     ...
     * 组件停止，这个时期会触发三个生命周期事件：BEFORE_STOP_EVENT，STOP_EVENT，AFTER_STOP_EVENT
     * @exception LifecycleException if this component detects a fatal error
