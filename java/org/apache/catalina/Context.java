 ...
 * <p>
 *
 * <b>Context<b> 是一个容器，它表示一个 servlet 上下文，因此是 Catalina servlet 引擎中的一个单独的 Web 应用程序。
 * 因此，它在 Catalina 的几乎所有部署中都很有用（即使连接到 Web 服务器（例如 Apache）的连接器使用 Web 服务器的设施
 * 来识别适当的 Wrapper 来处理此请求。它还提供了一种方便的机制来使用拦截器查看此特定 Web 应用程序处理的每个请求。
 * <p> 附加到 Context 的父容器通常是 Host，但可能是其他一些实现，或者如果不需要，可以省略。
 * <p> 附加到 Context 的子容器上下文通常是 Wrapper 的实现（代表单个 servlet 定义）。<p>
 *
...
public interface Context extends Container, ContextBind {
   ...
   /**
    ...
    /**
     * Will the parsing of web.xml and web-fragment-1.xml files for this Context
    ...
    /**
     * Controls whether the parsing of web.xml and web-fragment-1.xml files for
    ...
    /**
     * Controls whether the parsing of web.xml and web-fragment-1.xml files
    ...
    /**
     * Will the parsing of web.xml, web-fragment-1.xml, *.tld, *.jspx, *.tagx and
    ...
    /**
     * Controls whether the parsing of web.xml, web-fragment-1.xml, *.tld, *.jspx,
     ...
     * efficient but has the side effect of confirming that the context path is
     * valid.
     * 确定对web应用程序上下文根的请求是否将由Mapper重定向(添加尾斜杠)。这是更有效的，但有副作用，确认上下文路径是有效的
