...
public class Cache {
    ...
    private boolean noCache(String path) {
         ...
         * 不要缓存classes，类加载器会处理它，也不要缓存jars，资源set对象会处理它
         */
        boolean matchClass = path.endsWith(".class") && (path.startsWith("/WEB-INF/classes/") || path.startsWith("/WEB-INF/lib/"));
        boolean matchJar= path.startsWith("/WEB-INF/lib/") && path.endsWith(".jar");
        if (matchClass || matchJar) {
            return true;
        }
        return false;
    }
