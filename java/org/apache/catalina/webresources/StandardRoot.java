...
public class StandardRoot extends LifecycleMBeanBase implements WebResourceRoot {
    ...
    {
        /**
         * preResources：是context.xml中定义的 preResources 资源
         * mainResources：代表从web应用或者目录，war包中解析出来的
         * classResources：这个资源就是解析出来的class的资源
         * jarResources：Servlet规范定制的WEB-INF/lib下面的jar包的资源集合
         * postResources：是context.xml中定义的 postResources资源
         */
    ...
    /**
     * 当Tomcat需要对资源进行查找的时候，StandardRoot会首先找到这几个资源集合，
     * 从这几个资源集合中逐个去遍历对应的WebResourceSet，如果找到path，那么就由WebResourceSet去创建相应类型的WebResource：
     */
    protected WebResource getResource(String path, boolean validate,
        ...
        if (isCachingAllowed()) { // 默认是开启缓存的，提高查找性能
            /**缓存中取*/
    ...
    protected final WebResource getResourceInternal(String path,
        ...
        /**
         * 上述的资源查找的顺序，是严格符合Servlet规范的，这就会有同名路径的问题了，需要在编写应用的时候注意；
         * 另外，前面已经提过，在初始化资源的时候，如果开启Cache开关，
         * StandardRoot每一次不会都去实际去遍历与查询，直接从Cache中进行查找；
         */
        for (List<WebResourceSet> list : allResources) {
            for (WebResourceSet webResourceSet : list) {
                if (!useClassLoaderResources &&  !webResourceSet.getClassLoaderOnly() ||
                        useClassLoaderResources && !webResourceSet.getStaticOnly()) {
                    /**从当前资源jar集合中找到path（class）对应的WebResource*/
    ...
    protected void startInternal() throws LifecycleException {
        ...
        main = createMainResourceSet(); // 构建主资源集
