...
public abstract class AbstractHttp11Protocol<S> extends AbstractProtocol<S> {
    ...
    public AbstractHttp11Protocol(AbstractEndpoint<S,?> endpoint) {
        ...
        /**new一个处理当前连接请求的handler*/
        ConnectionHandler<S> cHandler = new ConnectionHandler<>(this);
        /**给当前的应用层协议设置请求连接处理器*/
        setHandler(cHandler);
        /**
         * endpoint是传输层tcp协议，接收客户端socket，而cHandler是
         * 通过与其关联的应用层协议protocol对象的的HTTP协议处理器processor来
         * 对请求进行处理转发
         */
    ...
    public void init() throws Exception {
        System.out.println("======> ProtocolHandler(AbstractHttp11Protocol) init.");
        // Upgrade protocols have to be configured first since the endpoint
        // init (triggered via super.init() below) uses this list to configure
        // the list of ALPN protocols to advertise
        /**
         * 如果我们使用https，可以升级协议为h2,在server.xml中配置如下：
         * <Connector>
         *     <UpgradeProtocol className="org.apache.coyote.http2.Http2Protocol" />
         * </Connector>
         *
         * {@link org.apache.coyote.http2.Http2Protocol}
         */
        for (UpgradeProtocol upgradeProtocol : upgradeProtocols) {
            configureUpgradeProtocol(upgradeProtocol);
        }

        super.init(); // 调用父类AbstractProtocol的init方法，其会初始化tcp/ip协议通信Endpoint
