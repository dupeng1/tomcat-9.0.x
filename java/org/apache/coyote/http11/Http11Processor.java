...
public class Http11Processor extends AbstractProcessor {
    ...
    /**这个方法很关键，Processor接收来自EndPoint的Socket，
     * 读取字节流解析成Tomcat Request和Response对象，
        ...
        while (!getErrorState().isError() && keepAlive && !isAsync() && upgradeToken == null &&
            ...
            try {
                ...
                if (protocol.isPaused()) {
                    ...
                    /** 每次处理一个请求就得重新获取一下请求头和cookie的最大限制*/
            ...
            if (getErrorState().isIoAllowed()) {
                ...
                try {
                    /**处理请求前准备工作（包括了从上一步解析的headers读取connection的属性）*/
                    prepareRequest();
                } catch (Throwable t) {
                    ExceptionUtils.handleThrowable(t);
                    if (log.isDebugEnabled()) {
            ...
            // Process the request in the adapter 使用合适的adapter处理请求（将request交给servlet应用）
            if (getErrorState().isIoAllowed()) {
                try {
                    rp.setStage(org.apache.coyote.Constants.STAGE_SERVICE);
                    /**获取adapter对象（CoyoteAdapter）并处理request*/
    ...
    private void prepareRequest() throws IOException {
        ...
        // Check connection header 检查Connection头
        MessageBytes connectionValueMB = headers.getValue(Constants.CONNECTION);
        if (connectionValueMB != null && !connectionValueMB.isNull()) {
            Set<String> tokens = new HashSet<>();
            TokenList.parseTokenList(headers.values(Constants.CONNECTION), tokens);
            /**如果读取Connection的值为close，ka就是false，反之就是长连接true*/
        ...
        // Validate host name and extract port if present 验证主机名并提取端口(如果存在)
