...
public abstract class AbstractProtocol<S> implements ProtocolHandler,MBeanRegistration {
     ...
     * port binding. 计数器，用于使用自动端口绑定为连接器生成唯一的JMX名称。
     */
     ...
     * Endpoint etc.).
     * 提供低级网络 IO 的 Endpoint -
     * 必须与 ProtocolHandler 实现匹配（ProtocolHandler 如果使用使用 NIO，则对应需要 NIO Endpoint等）
     * 即协议处理器包含endpoint（tcp/ip）和processor（http/ajp）
     */
     ...
     *
     * 将保留在缓存中并在后续请求中重用的空闲处理器的最大数量。
     * 默认值是200。值-1表示无限制。在无限制的情况下，
     * 理论上缓存的Processor对象的最大数量是{@link #getMaxConnections()}，
     * 尽管它通常更接近{@link #getMaxThreads()}。
     */
    ...
    /**设置协议的最大线程数，实际上设置的是它的小弟endpoint的maxThreads变量*/
    ...
    public void init() throws Exception {
        ...
        /**客户端与服务端端点初始化（nio 服务端套接字通道初始化、并设置阻塞）*/
        endpoint.init();
    }
    ...
    public void start() throws Exception {
        System.out.println("======> Protocol（AbstractProtocol） start.");
        if (getLog().isInfoEnabled()) {
        ...
        endpoint.start(); // 启动端点
        /***
         * scheduleWithFixedDelay 方法用于创建一个固定延迟的定时任务，在每次任务执行完成后等待一段固定的延迟时间，然后再次执行任务。
         * 每60秒执行一次异步任务
         */
     ...
     * 此方法的名称起源于Servlet 3.0异步处理，但随着时间的推移演变为表示独立于套接字读写超时触发的超时。
     * 总结来说，startAsyncTimeout() 方法在 AbstractProtocol 类中用于启动异步操作的超时监视，
     * 以确保异步操作不会无限期地挂起，并提供超时处理逻辑的支持。
     */
    protected void startAsyncTimeout() {
        if (timeoutFuture == null || timeoutFuture.isDone()) {
            ...
            /** scheduleAtFixedRate 方法用于创建一个固定速率的定时任务，无论任务的执行时间长短，都会按照设定的时间间隔重复执行任务。*/
    ...
    protected static class ConnectionHandler<S> implements AbstractEndpoint.Handler<S> {
        ...
        /**
         * 持有一个应用层协议对象，值设置在
         * {@link org.apache.coyote.http11.AbstractHttp11Protocol#AbstractHttp11Protocol(AbstractEndpoint<S,?>)}
         */
        private final AbstractProtocol<S> proto;
        private final RequestGroupInfo global = new RequestGroupInfo();
        private final AtomicLong registerCount = new AtomicLong(0);
        /**享元模式，复用上一个请求连接关联的应用层协议Processor对象*/
        ...
        /**当前的handler要拿到proto对应的processor，所以这里proto一定是要有东西的*/
        ...
        public SocketState process(SocketWrapperBase<S> wrapper, SocketEvent status) {
            ...
            /**新的请求连接进来后，nioSocketWrapper是没有关联对应的processor的*/
            ...
            try {
                ...
                /**
                 * tomcat启动后，初次有请求连接进来后，recycledProcessors是empty的，所以要new
                 */
                if (processor == null) {
                    /**
                     * 如果处理器空的话，那就基于server.xml配置的connector的protocol协议创建一个处理器
                     * 一般我们用的是Http11Processor（默认提供的）
                     */
                    processor = getProtocol().createProcessor();
                    register(processor); // 将processor注册到MBServer
                    if (getLog().isDebugEnabled()) {
                        getLog().debug(sm.getString("abstractConnectionHandler.processorCreate", processor));
                    }
                }
