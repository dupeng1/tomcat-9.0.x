 ...
 *
 * 该类用于表示 HTTP 消息中的字节子数组。它表示所有的 request / response 元素。字节转换被延迟并缓存。所有东西都是可回收的。
 *
