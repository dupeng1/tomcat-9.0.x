...
/**
* Callback handling a web-fragment.xml descriptor. 处理web-fragment-1.xml描述符的回调。
*/
public class FragmentJarScannerCallback implements JarScannerCallback {
    ...
    /**将扫描到的符合FRAGMENT_LOCATION规格的webXml添加进fragments中*/
    ...
    public void scan(Jar jar, String webappPath, boolean isWebapp) throws IOException {
        ...
        try {
            /**
             * Only web application JARs are checked for web-fragment.xml files.
             *  web-fragment.xml files don't need to be parsed if they are never going to be used.
             * 只有被认作是web应用程序的jars（这种情况实际上只会发生在web部署描述符片段所在的jar）
             * 才会去检查jar包中有无 META-INF/web-fragment.xml文件
             * 如果有，就拿到inputStream
             */
            ...
            /**不管jar包是不是含有${FRAGMENT_LOCATION}文件，解不解析最终都会放到fragments映射中*/
    ...
    private void addFragment(WebXml fragment, URL url) {
        ...
        /**如果片段映射中已经包含了当前片段的名称，则将之前存在的fragment对象标记为重复*/
    ...
    public void scanWebInfClasses() {
        ...
        // handle multiple web-fragment-1.xml files.
    }
