 ...
 * 创建新对象并将其推入对象堆栈的规则实现。当元素完成时，对象将被弹出
 */
...
public class ObjectCreateRule extends Rule {
    ...
    public void begin(String namespace, String name, Attributes attributes)
        ...
        boolean bServerNode = "Server".equals(name);
        if (bServerNode){
            System.out.println("======> ObjectCreateRule#begin, realClassName = "+realClassName);
        }
        ...
        /**
         * 这里获取的类加载器是线程上下文中的，也就是Bootstrap类中我们设置的CatalinaClassLoader实例
         * {@link Bootstrap#init()}
         */
        ClassLoader catalinaClassLoader = digester.getClassLoader();
        Class<?> clazz = catalinaClassLoader.loadClass(realClassName);
        Object instance = clazz.getConstructor().newInstance();
        if (bServerNode){
            /**需要注意的是，xml解析不仅仅是反射创建Server和Service组件，当然还有Engine和Host等容器还有其他*/
            System.out.println("======> Server节点通过反射完成了实例化");
        }
