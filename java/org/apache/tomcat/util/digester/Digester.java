...
public class Digester extends DefaultHandler2 {
     ...
     * 获取实例化Tomcat内部生命周期组件对象的类加载器（从线程上下文中拿，拿的是server/catalina类加载器）
     * <ul>
     ...
     * </ul>
     * 1. 类加载器来自于外部设置，走setClassLoader方法（如果这个空就走2和3）
     * 2. 类加载器来自于Tomcat启动类Bootstrap中设置的线程上下文（catalinaLoader,前提是useContextClassLoader设置为true）
     * 3. 类加载器就是加载Digester类的classLoader，默认是AppClassLoader
    ...
    public ClassLoader getClassLoader() {
        ...
        if (this.useContextClassLoader) {
            /**
             * 如果 {@link Bootstrap#init()} 中设置了
             * Thread.currentThread().setContextClassLoader(catalinaLoader);
             * 那这里的classLoader就是它，否则就是AppClassLoader
             */
     ...
     * 我们将使用SAXParser来解析输入流。如果创建解析器有问题，则返回<code>null<code>。
     */
     ...
     * 确定是否使用ContextClassLoader
     * (通过调用<code>Thread.currentThread(). getcontextclassloader ()<code>找到的)
     * 来解析在各种规则中定义的类。如果不使用Context ClassLoader，则类加载默认使用调用类ClassLoader。
    ...
    /**
     * Determine if an attribute is a fake attribute. 确定属性是否为假属性。
