...
public class Nio2Endpoint extends AbstractJsseEndpoint<Nio2Channel,AsynchronousSocketChannel> {
    ...
    public void bind() throws Exception {
        ...
        /**相比于NioEndpoint类，当前这个类是异步的，serverSock是异步类型的服务端套接字通道*/
        serverSock = AsynchronousServerSocketChannel.open(threadGroup);
        socketProperties.setProperties(serverSock);
        InetSocketAddress addr = new InetSocketAddress(getAddress(), getPortWithOffset());
        serverSock.bind(addr, getAcceptCount());
        System.out.println("======> Nio2Endpoint bind port - "+addr.getPort());
     ...
     * 这个类等价于Worker，但将简单应用于外部的Executor线程池。
     */
